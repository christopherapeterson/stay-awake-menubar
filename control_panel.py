...
# Palette — matches scripts/make_app_icon.py.
_TOP = NSColor.colorWithCalibratedRed_green_blue_alpha_(247 / 255, 178 / 255, 88 / 255, 1.0)
_BOT = NSColor.colorWithCalibratedRed_green_blue_alpha_(236 / 255, 130 / 255, 74 / 255, 1.0)
_CREAM = NSColor.colorWithCalibratedRed_green_blue_alpha_(1.0, 244 / 255, 224 / 255, 1.0)
_CREAM_DIM = NSColor.colorWithCalibratedRed_green_blue_alpha_(1.0, 244 / 255, 224 / 255, 0.72)
_ORANGE_TEXT = NSColor.colorWithCalibratedRed_green_blue_alpha_(0.80, 0.42, 0.16, 1.0)
...
class ControlPanel(NSObject):
    ...
    def _build(self):
        ...
        self._status_label = _label("", NSMakeRect(10, _H - 222, _W - 20, 22),
                                    NSFont.systemFontOfSize_(13.5), _CREAM_DIM)
        ...
        root.addSubview_(_label("O R   S T A Y   A W A K E   F O R",
                                ...
                                cap_font, _CREAM_DIM, kern=1.0))

        # Preset pills.
        pill_font = (NSFont.fontWithName_size_("AvenirNext-DemiBold", 14)
                     or NSFont.boldSystemFontOfSize_(14))
        pill_bg = NSColor.colorWithCalibratedRed_green_blue_alpha_(1, 1, 1, 0.22)
        ...
        for label, secs in presets:
            b = _pill(label, NSMakeRect(x, _H - 388, w, 42), self,
                      "presetClicked:", 21, pill_font, _CREAM, pill_bg)
            b.setTag_(secs)
            root.addSubview_(b)
        ...
        root.addSubview_(_label("Also lives in your menu bar — look for the cup",
                                ...
                                NSFont.systemFontOfSize_(11), _CREAM_DIM))
    ...
    def _refresh(self):
        ...
        if sa.active:
            ...
            self._toggle_btn.setAttributedTitle_(
                _attr_title("Turn Off", toggle_font, _CREAM))
            self._toggle_btn.layer().setBackgroundColor_(
                NSColor.colorWithCalibratedRed_green_blue_alpha_(1, 1, 1, 0.24).CGColor())
        else:
            status = "○  Off — your Mac can sleep normally"
            self._toggle_btn.setAttributedTitle_(
                _attr_title("Turn On", toggle_font, _ORANGE_TEXT))
            self._toggle_btn.layer().setBackgroundColor_(_CREAM.CGColor())
        self._status_label.setAttributedStringValue_(
            _attr_title(status, NSFont.systemFontOfSize_(13.5), _CREAM_DIM))
        # Tick the active preset pill.
        for b in self._preset_btns:
            on = sa.active and sa._active_seconds == b.tag()
            b.layer().setBackgroundColor_(
                NSColor.colorWithCalibratedRed_green_blue_alpha_(
                    1, 1, 1, 0.45 if on else 0.22).CGColor())
